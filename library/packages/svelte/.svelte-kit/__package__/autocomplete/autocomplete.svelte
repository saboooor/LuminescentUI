...
<script>export let getValues = void 0;
export let transformData = void 0;
export let renderChild = void 0;
...
function onUpdateFn_0(..._args) {
  fetchVals(inputVal).then((newVals) => {
    ...
    suggestions = newVals.filter(
      (data) => transform(data).toLowerCase().includes(inputVal.toLowerCase())
    );
  });
}
$:
